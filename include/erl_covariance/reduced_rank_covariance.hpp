...
namespace erl::covariance {
    ...
    public:
        ...
        private:
            ...
            Matrix m_frequencies_;              // frequencies for the basis functions, each column is a frequency vector, (fx, fy, fz, ...)
            Vector m_spectral_densities_;       // spectral densities for the basis functions, (s1, s2, s3, ...)
            Vector m_inv_spectral_densities_;   // inverse spectral densities for the basis functions, (1/s1, 1/s2, 1/s3, ...)

        public:
            void
            BuildSpectralDensities(const std::function<Vector(const Vector & /*freq_squared_norm*/)> &kernel_spectral_density_func);
    ...
    protected:
        ...
        Vector m_coord_origin_;  // origin of the coordinate system for the basis functions
        Matrix m_mat_k_;         // accumulated kernel matrix approximation
        Vector m_vec_alpha_;     // accumulated alpha vector approximation
    ...
    public:
        ...
        std::pair<long, long>
        ComputeKtrainWithGradient(const Eigen::Ref<const Matrix> &mat_x, long num_samples, Eigen::VectorXl &vec_grad_flags, Matrix &mat_k, Vector &vec_alpha)
            const override;
        ...
        BuildSpectralDensities() {
            m_setting_->BuildSpectralDensities([this](const Vector &freq_squared_norm) -> Vector { return ComputeSpectralDensities(freq_squared_norm); });
            const long e = m_setting_->num_basis.prod();
            if (m_setting_->accumulated) {
                if (m_mat_k_.size() == 0) { m_mat_k_ = Matrix::Zero(e, e); }
                if (m_vec_alpha_.size() == 0) { m_vec_alpha_ = Vector::Zero(e); }
            }
        }

        [[nodiscard]] virtual Vector
        ComputeSpectralDensities(const Vector &freq_squared_norm) const = 0;

        [[nodiscard]] Matrix
        ComputeEigenFunctions(const Eigen::Ref<const Matrix> &mat_x, long dims, long num_samples) const;

        [[nodiscard]] Matrix
        ComputeEigenFunctionsWithGradient(const Eigen::Ref<const Matrix> &mat_x, long dims, long num_samples, Eigen::VectorXl &vec_grad_flags) const;
        ...
        void
        SetCoordOrigin(const Vector &coord_origin) {
