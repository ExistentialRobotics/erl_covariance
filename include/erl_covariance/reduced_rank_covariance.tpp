...
void
ReducedRankCovariance<Dtype>::Setting::BuildSpectralDensities(const std::function<Vector(const Vector &)> &kernel_spectral_density_func) {
...

    {
        ...
        for (long i = 0; i < x_dim; ++i) {
            ...
            Matrix frequencies = Vector::LinSpaced(dim_size, f, static_cast<Dtype>(dim_size) * f)  //
                                     .transpose()
                                     .replicate(stride, n_copies / stride);
            m_frequencies_.row(i) << frequencies.reshaped(total_size, 1).transpose();
        }

        Vector freq_squared_norm = m_frequencies_.colwise().squaredNorm();
        if (max_num_basis > 0 && max_num_basis < freq_squared_norm.size()) {
            ...
            Vector freq_squared_norm_new = freq_squared_norm(indices);
            Matrix frequencies_new = m_frequencies_(Eigen::indexing::all, indices);
...
template<typename Dtype>
YAML::Node
ReducedRankCovariance<Dtype>::Setting::Encode() const {
    YAML::Node node = Super::Setting::Encode();
    node["max_num_basis"] = max_num_basis;
    node["num_basis"] = num_basis;
...
template<typename Dtype>
bool
ReducedRankCovariance<Dtype>::Setting::Decode(const YAML::Node &node) {
    if (!Super::Setting::Decode(node)) { return false; }
    max_num_basis = node["max_num_basis"].as<long>();
    num_basis = node["num_basis"].as<Eigen::VectorXl>();
    boundaries = node["boundaries"].as<Vector>();
    accumulated = node["accumulated"].as<bool>();
    return true;
}

template<typename Dtype>
std::pair<long, long>
ReducedRankCovariance<Dtype>::ComputeKtrain(const Eigen::Ref<const Matrix> &mat_x, long num_samples, Matrix &mat_k, Vector &vec_alpha) const {
    ...
    const Matrix &phi = ComputeEigenFunctions(mat_x, dims, num_samples);  // (N, E)
    auto mat_k_block = mat_k.topLeftCorner(e, e);
    mat_k_block << phi.transpose() * phi;
    auto vec_alpha_block = vec_alpha.head(e);
    vec_alpha_block << phi.transpose() * Vector(vec_alpha.head(num_samples));
...
ReducedRankCovariance<Dtype>::ComputeKtrain(
    ...
    const Matrix phi = ComputeEigenFunctions(mat_x, dims, num_samples);  // (num_samples, e)
    const Vector inv_sigmas = vec_var_y.head(num_samples).cwiseInverse();
    const Vector y = vec_alpha.head(num_samples);
    const Vector inv_spectral_densities = m_setting_->GetInvSpectralDensities();
    const bool accumulated = m_setting_->accumulated;

    Vector inv_sigmas_phi_i(num_samples);
    auto acc_mat_k = const_cast<Matrix &>(m_mat_k_);
    auto acc_vec_alpha = const_cast<Vector &>(m_vec_alpha_);
...
ReducedRankCovariance<Dtype>::ComputeKtrainWithGradient(
    ...
    const Matrix phi = ComputeEigenFunctionsWithGradient(mat_x, dims, num_samples, vec_grad_flags);  // (m, e)
    const long m = phi.rows();
    const Vector y = vec_alpha.head(m);
    const Vector inv_spectral_densities = m_setting_->GetInvSpectralDensities();
    const bool accumulated = m_setting_->accumulated;
...
template<typename Dtype>
typename ReducedRankCovariance<Dtype>::Matrix
ReducedRankCovariance<Dtype>::ComputeEigenFunctions(const Eigen::Ref<const Matrix> &mat_x, const long dims, const long num_samples) const {

    const Matrix &frequencies = m_setting_->GetFrequencies();
    ...
    Matrix eigen_functions(num_samples, e);
    const Dtype alpha = 1.0 / std::sqrt(m_setting_->boundaries.head(dims).prod());
    Vector coord_origin = m_coord_origin_;
    if (coord_origin.size() == 0) { coord_origin = Vector::Zero(dims); }
...
ReducedRankCovariance<Dtype>::ComputeEigenFunctionsWithGradient(
    ...
    const Dtype alpha = 1.0 / std::sqrt(m_setting_->boundaries.head(dims).prod());
    Matrix eigen_functions(num_samples + n_grad * dims, e);
    Vector coord_origin = m_coord_origin_;
    if (coord_origin.size() == 0) { coord_origin = Vector::Zero(dims); }
...
ReducedRankCovariance<Dtype>::operator==(const ReducedRankCovariance &other) const {
    ...
    if (m_mat_k_.size() != other.m_mat_k_.size() || std::memcmp(m_mat_k_.data(), other.m_mat_k_.data(), m_mat_k_.size() * sizeof(Dtype)) != 0) { return false; }
