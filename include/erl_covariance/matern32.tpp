...
namespace erl::covariance {
    ...
    Matern32<Dtype, Dim>::ComputeKtrain(const Eigen::Ref<const MatrixX> &mat_x, const long num_samples, MatrixX &mat_k, MatrixX & /*mat_alpha*/) {
    ...
    template<typename Dtype, int Dim>
    [[nodiscard]] std::pair<long, long>
    Matern32<Dtype, Dim>::ComputeKtrain(const Eigen::Ref<const MatrixX> &mat_x, const long num_samples, MatrixX &mat_k) {
        MatrixX mat_alpha;
                ...
                if (grad_flags[j]) {
                    ...
                    if (grad_flags[i]) {
                        ...
                        for (long k = 0, ki = grad_flags[i], kj = grad_flags[j]; k < dim; ++k, ki += n_grad, kj += n_grad) {
                            ...
                            k_kj_ki = InlineMatern32Gradx1BetweenGradx2<Dtype>(a, b, 1., dxk, dxk, r, b_exp_term);  // cov(df_j/dx_k, df_i/dx_k)
                            ...
                            for (long l = k + 1, li = ki + n_grad, lj = kj + n_grad; l < dim; ++l, li += n_grad, lj += n_grad) {
                                ...
                                k_kj_li = InlineMatern32Gradx1BetweenGradx2<Dtype>(a, b, 0., dxk, dxl, r, b_exp_term);  // cov(df_j/dx_k, df_i/dx_l)
            ...
            for (long i = 0, ki_init = num_samples1; i < num_samples1; ++i) {
                ...
                if (predict_gradient) {
                    for (long k = 0, ki = ki_init, kj = j + num_samples2; k < dim; ++k, ki += n_grad, kj += num_samples2) {
                        ...
                        mat_k_kj_ptrs[k][ki] = InlineMatern32Gradx1BetweenGradx2<Dtype>(a, b, 1., dxk, dxk, r, b_exp_term);
                        ...
                        for (long l = k + 1, li = ki + n_grad, lj = kj + num_samples2; l < dim; ++l, li += n_grad, lj += num_samples2) {
                            ...
                            k_ki_lj = InlineMatern32Gradx1BetweenGradx2<Dtype>(a, b, 0., dxk, dxl, r, b_exp_term);  // cov(df1_i/dx_k, df2_j/dx_l)
