...
static Dtype
InlineMatern32(const Dtype alpha, const Dtype a, const Dtype r, const Dtype exp_term) {
...
template<typename Dtype>
static Dtype
InlineMatern32Gradx1BetweenGradx2(const Dtype a, const Dtype b, const Dtype delta, const Dtype dx_1, const Dtype dx_2, const Dtype r, const Dtype b_exp_term) {
...
Matern32<Dim, Dtype>::ComputeKtrain(const Eigen::Ref<const Matrix> &mat_x, const long num_samples, Matrix &mat_k, Vector & /*vec_alpha*/) const {
    ...
    const Dtype a2 = std::sqrt(3.) / Super::m_setting_->scale;
    const Dtype a1 = a2 * alpha;
...
Matern32<Dim, Dtype>::ComputeKtest(
    ...
    for (long j = 0; j < num_samples2; ++j) {
        ...
        for (long i = 0; i < num_samples1; ++i) {
            ...
            col_j_ptr[i] = InlineMatern32(alpha, a1, r, std::exp(-a2 * r));
        }
    }
...
Matern32<Dim, Dtype>::ComputeKtrainWithGradient(
    ...
    for (long j = 0; j < num_samples; ++j) {
        ...
        for (long i = j + 1; i < num_samples; ++i) {
            ...
            const Dtype exp_term = std::exp(-a2 * r);
            ...
            if (grad_flags[j]) {
                ...
                if (grad_flags[i]) {
                    ...
                    for (long k = 0, ki = grad_flags[i], kj = grad_flags[j]; k < dim; ++k, ki += n_grad, kj += n_grad) {
                        ...
                        k_kj_ki = InlineMatern32Gradx1BetweenGradx2<Dtype>(a2, b, 1., dxk, dxk, r, b_exp_term);  // cov(df_j/dx_k, df_i/dx_k)
                        ...
                        for (long l = k + 1, li = ki + n_grad, lj = kj + n_grad; l < dim; ++l, li += n_grad, lj += n_grad) {
                            ...
                            k_kj_li = InlineMatern32Gradx1BetweenGradx2<Dtype>(a2, b, 0., dxk, dxl, r, b_exp_term);  // cov(df_j/dx_k, df_i/dx_l)
...
Matern32<Dim, Dtype>::ComputeKtrainWithGradient(
    ...
    for (long j = 0; j < num_samples; ++j) {
        ...
        if (grad_flags[j]) {
            ...
            for (long k = 0, kj = grad_flags[j]; k < dim; ++k, kj += n_grad) {
                mat_k_kj_ptrs[k][kj] = b + vec_var_grad[j];  // mat_k(kj, kj) = cov(df_j/dx_k, f_j)
                mat_k_kj_ptrs[k][j] = 0.;                    // mat_k(j, kj) = cov(df_j/dx_k, f_j)
                mat_k_j_ptr[kj] = 0.;                        // mat_k(kj, j) = cov(df_j/dx_k, f_j)
                for (long l = k + 1, lj = kj + n_grad; l < dim; ++l, lj += n_grad) {
...
Matern32<Dim, Dtype>::ComputeKtestWithGradient(
    ...
    const Dtype a2 = std::sqrt(3.) / Super::m_setting_->scale;
    const Dtype a1 = a2 * Super::m_setting_->alpha;
    const Dtype b = a2 * a2 * Super::m_setting_->alpha;
    // buffer to store the difference between x1_i and x2_j
    Eigen::Vector<Dtype, Dim> diff_ij;          // avoid memory allocation on the heap
    Eigen::Vector<Dtype *, Dim> mat_k_kj_ptrs;  // avoid memory allocation on the heap
    if constexpr (Dim == Eigen::Dynamic) {
    ...
    for (long j = 0; j < num_samples2; ++j) {
        ...
        for (long i = 0, ki_init = num_samples1; i < num_samples1; ++i) {
            ...
            r = std::sqrt(r);                                                            // (mat_x1.col(i) - mat_x2.col(j)).norm();
            const Dtype exp_term = std::exp(-a2 * r);                                    // exp(-a2 * r), which is frequently used
            const Dtype b_exp_term = b * exp_term;                                       // b * exp(-a2 * r)
            mat_k_j_ptr[i] = InlineMatern32(Super::m_setting_->alpha, a1, r, exp_term);  // cov(f1_i, f2_j)
            if (predict_gradient) {
                for (long k = 0, kj = j + num_samples2; k < dim; ++k, kj += num_samples2) {       // cov(f1_i, df2_j/dx_k)
                    mat_k_kj_ptrs[k][i] = InlineMatern32X1BetweenGradx2(diff_ij[k], b_exp_term);  // mat_k(i, kj)
                }
            }
            ...
            if (predict_gradient) {
                for (long k = 0, ki = ki_init, kj = j + num_samples2; k < dim; ++k, ki += n_grad, kj += num_samples2) {
                    mat_k_j_ptr[ki] = -mat_k_kj_ptrs[k][i];  // mat_k(ki, j) = -mat_k(i, kj), i.e. cov(df1_i/dx_k, f2_j) = -cov(f1_i, df2_j/dx_k)

                    // between Dim-k and Dim-k
                    const Dtype &dxk = diff_ij[k];
                    // mat_k(ki, kj) = cov(df1_i/dx_k, df2_j/dx_k)
                    mat_k_kj_ptrs[k][ki] = InlineMatern32Gradx1BetweenGradx2<Dtype>(a2, b, 1., dxk, dxk, r, b_exp_term);
                    ...
                    for (long l = k + 1, li = ki + n_grad, lj = kj + num_samples2; l < dim; ++l, li += n_grad, lj += num_samples2) {
                        ...
                        k_ki_lj = InlineMatern32Gradx1BetweenGradx2<Dtype>(a2, b, 0., dxk, dxl, r, b_exp_term);  // cov(df1_i/dx_k, df2_j/dx_l)
                        mat_k_kj_ptrs[k][li] = k_ki_lj;                                                          // mat_k(li, kj) = cov(df1_i/dx_l, df2_j/dx_k)
                    }
                }
            } else {
                for (long k = 0, ki = ki_init, kj = j + num_samples2; k < dim; ++k, ki += n_grad, kj += num_samples2) {
                    mat_k_j_ptr[ki] = -InlineMatern32X1BetweenGradx2(diff_ij[k], b_exp_term);  // cov(df1_i/dx_k, f2_j)
                }
            }
            ++ki_init;
        }
