...
TEST(ReducedRankMatern32_2d, Copy) {
    ...
    std::shared_ptr<ReducedRankMatern32_2d::Factory::BaseType> cov_new = std::make_shared<ReducedRankMatern32_2d>(setting);
...
TEST(ReducedRankMatern32_2f, EigenFunctions) {
    ...
    Eigen::MatrixXf xy = LoadEigenMatrixFromBinaryFile(gtest_src_dir / "train_xy.dat").cast<float>();
    Eigen::MatrixXf phi_gt = LoadEigenMatrixFromBinaryFile(gtest_src_dir / "train_phi.dat").cast<float>();  // eigen functions
    Eigen::VectorXf spect_density_gt = LoadEigenMatrixFromBinaryFile(gtest_src_dir / "spect_density.dat").diagonal().cast<float>();
    ...
    for (long i = 0; i < phi.cols(); ++i) {
        ...
        for (long j = 0; j < diff.size(); ++j) { ASSERT_TRUE(std::abs(diff[j]) < kTolerance) << "i = " << i << ", j = " << j << ", diff = " << diff[j]; }
    }
}
