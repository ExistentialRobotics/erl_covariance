...
matern32_new(const std::shared_ptr<erl::covariance::Matern32_3d::Setting> &setting, const Eigen::MatrixXd &mat_x, const Eigen::VectorXl &vec_grad_flags) {
    ...
    const double a2 = std::sqrt(3.) / setting->scale;
...
#pragma omp simd
            ...
            // auto diff = diffs_grad.col(k);
            // diff << mat_x_with_grad.col(k).array() - mat_x(k, j);
            // k_mat_col_j.segment(ki, n_grad) = -diff.array() * b_exp_terms.array();
        }

        // lower triangular part of cov(df_i/dx_k, df_j/dx_l)
        if (!vec_grad_flags[j]) { continue; }
        for (long l = 0, lj = grad_indices[j], rows_grad = n + n_grad; l < dim; ++l, lj += n_grad, rows_grad += n_grad) {
            ...
            if (const long m_grad = rows_grad - lj - 1; m_grad > 0) {
                ...
                k_mat_col_lj.segment(lj + 1, m_grad) = (1.0 - a2 * dx.array().square() / r.array()) * b_exp_term.array();
            }
            ...
            for (long k = l + 1, start = rows_grad; k < dim; ++k, start += n_grad) {
                ...
                for (long i = 0; i < n_grad; ++i) {
                    ...
                    cov_ki_lj_ptr[i] = -a2 * dx_kl / diff_norms_grad_ptr[i] * b_exp_terms_ptr[i];
                }
            }
        }
    }
...
TEST(Matern32, 3D) {
    ...
    Eigen::MatrixXd k_mat2;
    {
        ...
        (void) timer;
        for (long i = 0; i < n_tests; ++i) { k_mat2 = matern32_new(kernel_setting, mat_x, vec_grad_flags); }
    }
    ...
    Eigen::MatrixXd k_mat3;
    {
        ...
        (void) timer;
        for (long i = 0; i < n_tests; ++i) { k_mat3 = matern32_new2(kernel_setting, mat_x, vec_grad_flags); }
    }
    ...
    std::cout << "vec_grad_flags = " << vec_grad_flags.transpose() << ", n_grad = " << vec_grad_flags.cast<int>().sum() << std::endl;
    for (long j = 0; j < cols; ++j) {
        for (long i = 0; i < rows; ++i) {
            ASSERT_NEAR(k_mat1(i, j), k_mat2(i, j), 1.e-10) << "i = " << i << ", j = " << j << ", mat_x.cols() = " << mat_x.cols() << std::endl;
        }
    }
